...
| `apps/web`           | `@repo/database/enums`, `@repo/database/models`                   | `@repo/database/server` |
...
3. After `pnpm --filter web build`, this must print `0`:
   `grep -rlE "PrismaClient|@prisma/adapter|node:process" apps/web/dist/client | wc -l`
...
`z.enum(Role)`. Model *types* cross as `import type` only, to assert parity —
...
layer (`toContract` in `apps/api/src/modules/users/users.service.ts`).
...
That is deliberate. `apps/api` is CommonJS (NestJS + SWC) and `apps/web` is ESM,
while the generated client's `moduleFormat` picks *one* of `__dirname` /
